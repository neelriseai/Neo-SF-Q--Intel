...
| R0.4 | Verified change and obligation matrix — R0.4a/R0.4b/R0.4c local foundations implemented | R0.4a derives complete base/candidate input trees and ADD/MODIFY/DELETE from actual local-Git bytes. R0.4b re-verifies those bytes and current R0.2/R0.1 evidence and derives stable affected-path projections without caller seeds. R0.4c independently replays both trees into file inventory plus a typed, provenance-bound Salesforce semantic graph for explicitly supported source families, then recomputes node/edge deltas and tombstones. Package roots are descriptor-derived; unsupported source-root families fail closed; developer project indexes are never promoted to product evidence. Semantic-family completeness, operation-aware R0.4b consumption, build, obligation and execution gaps remain. Next add the R0.4d operation-aware mapper over both product graph sides, then trusted build outputs, complete impact/control obligations and exact-build execution receipts |
