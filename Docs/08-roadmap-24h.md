...
| R0.4 | Verified change and obligation matrix — R0.4a local-Git change-set foundation implemented | A policy-pinned producer derives the complete base/candidate input trees and ADD/MODIFY/DELETE delta from actual local-Git bytes without caller-supplied changed paths; secret/auth paths, unsupported modes, sparse/ambiguous state, capacity and capture races fail closed. This is `ANALYSIS_ONLY` input-tree evidence, not a verified build/deploy. Next bind a trusted build's toolchain/config/environment/output bytes, derive every material impact/control obligation from the current replayed scope, and require exact-build trusted execution receipts |
