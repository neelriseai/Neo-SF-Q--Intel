...
| R0.4 | Verified change and obligation matrix — R0.4a/R0.4b local foundations implemented | R0.4a derives complete base/candidate input trees and ADD/MODIFY/DELETE from actual local-Git bytes. R0.4b re-verifies those bytes and current R0.2/R0.1 evidence, maps each supported changed locator to a unique policy-defined graph anchor, and derives stable affected-path projections without caller seeds or subsets. Deletes, unmapped/ambiguous/unsafe locators, zero paths, stale or tampered evidence fail closed. Because no graph-producer receipt yet proves the graph came from the same Git tree, both foundations remain `ANALYSIS_ONLY` with explicit graph-input/change-seed gaps. Next add candidate/base graph-producer receipts, trusted build outputs, complete impact/control obligations and exact-build execution receipts |
