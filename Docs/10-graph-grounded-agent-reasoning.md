...
| Verified local change and graph-seed mapping | R0.4a derives complete base/candidate trees and byte changes. R0.4b independently replays the capture and current path evidence. R0.4c replays the exact bytes into independently rooted file inventory and a typed Salesforce Change Evidence Graph for supported descriptor-scoped source families. It materializes source-owned entities and relationships, preserves confirmed versus inferred facts, keeps byte provenance separate from semantic identity, and independently recomputes node/edge deltas and tombstones. Unknown package source families, unsafe or mutated files, malformed schemas and tampered receipts fail closed. Developer project indexes remain navigation aids and are never reasoning authority | Make R0.4d consume both product graph sides: ADD from candidate, DELETE from base and MODIFY from their exact semantic union. `SEMANTIC_SOURCE_FAMILY_COVERAGE_INCOMPLETE` and change-seed completeness remain blocking until additional source adapters and operation-aware replay are reviewed. Then bind trusted build/toolchain/output and exact-build test execution receipts before satisfying `CANDIDATE_BUILD_NOT_VERIFIED` |
