...
| Graph-grounded specialist | Isolated provider-neutral proposal/verifier directly replays A3/A4 inputs, validates ontology relation signatures and ordered citation bindings, forces abstention on unresolved conflicts, rejects unsafe captured output and emits only candidate/inferred analysis artifacts | Add the real OpenAI/Azure structured-output adapter with adapter-level hard cancellation, then consume replay-validated proposals in A6; quality remains insufficient until at least 20 adjudicated cases |
| Outcome memory | Run documents persist; outcome graph ports are schema-only | Append-only release/incident/human-correction records linked to exact snapshots and decisions |
