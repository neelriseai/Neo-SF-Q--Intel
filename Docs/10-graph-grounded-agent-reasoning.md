...
| Verified local change set | R0.4a uses a policy-pinned local-Git contract and producer to derive complete base and candidate input-tree manifests plus ADD/MODIFY/DELETE entries from actual bytes. Repository paths remain relative; caller changed paths, secret/auth paths, sparse/ambiguous state, unsupported modes, capacity overflow and capture races cannot yield an artifact. Rename inference is intentionally represented as delete plus add | Map every changed artifact to source-profile-derived graph seeds or a blocking unmapped-change gap; retain captured candidate bytes as the sole build input, then bind trusted build toolchain/config/environment/output manifests before satisfying `CANDIDATE_BUILD_NOT_VERIFIED` |
