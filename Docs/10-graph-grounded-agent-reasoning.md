...
| Verified local change and graph-seed mapping | R0.4a derives complete base/candidate trees and byte changes. R0.4b preserves current replay of the earlier path-evidence foundation. R0.4c replays exact bytes into independently rooted file inventory and a typed Salesforce Change Evidence Graph for supported source families under exactly one DX descriptor at arbitrary repository depth. Package roots resolve relative to each side's independently discovered descriptor while all repository files remain accounted. R0.4d current-replays that graph and derives side-qualified semantic seeds: ADD from candidate, DELETE from base with exact tombstones, and MODIFY from both independent versions. A separately pinned host pipeline composes and current-verifies these foundations without accepting caller scope, binds the unique candidate DX root to the configured nested app root, returns only a bounded receipt/measurement projection, sanitizes stage failures, prevents downstream execution and partial-artifact promotion after failure, and preserves `INCOMPLETE`/`ANALYSIS_ONLY`/release-false invariants. A fresh zero-scope POST exposes that projection without persistence or `AssuranceRun` contamination. Its separately owned dashboard panel independently bounds and runtime-decodes the response, replays exact stage transitions, permanent gaps, receipt lineage, timestamps and canonical digests, removes stale evidence and never changes assurance-run or release state. Ambiguous descriptors, project-root mismatch, path aliases, unsafe or overlapping package roots, unknown package source families, unsafe or mutated files, operation/side mismatches, missing tombstones and tampered receipts fail closed. Developer project indexes remain navigation aids and are never reasoning authority | Feed R0.4d side-qualified seeds into independently trusted BASE/CANDIDATE R0.2/R0.1 path replay. `SEMANTIC_SOURCE_FAMILY_COVERAGE_INCOMPLETE`, `GRAPH_INPUT_TREE_NOT_ATTESTED` and release-level change-seed completeness remain blocking until that integration and additional source adapters are reviewed. Then bind trusted build/toolchain/output and exact-build test execution receipts before satisfying `CANDIDATE_BUILD_NOT_VERIFIED` |
...
| Graph context compiler | Analysis-only foundation compiles replay-validated structural paths under pinned count/character/byte budgets, keeps unresolved fragments and semantic receipts candidate-only, exposes omissions/foundation gaps and now feeds the configured candidate-path specialist stages | Add typed seed-rationale receipts, deterministic depth/work-unit accounting, provider-specific token/time budgets, governed fragment resolution/redaction, prior outcomes and semantic-index receipts |
...
| Graph-grounded specialist | The provider-neutral proposal/verifier directly replays A3/A4 inputs. The verified candidate path now invokes configured OpenAI/Azure-capable structured-output specialist stages, binds complete request/graph identities, replays current governance, isolates failures, persists private replay captures and exposes only safe artifact/workflow hashes in the public view. Azure unsupported API versions fail closed before dispatch | Add adapter-level hard cancellation, independent restart replay over durable capture/consumption receipts, retry-safe CAS/checkpoint/service integration and at least 20 adjudicated quality cases |
