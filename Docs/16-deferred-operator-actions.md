...
| OA-008 | LWC candidate deploy/restore support | Before deploying the current local AUT locator-drift candidate to Salesforce | Extend the mutation authority, recovery preimage, restoration and reconciliation flow to cover the exact LWC HTML candidate mutation package, then run check-only and recovery rehearsal before any dispatch | Exact source package, check-only result, durable recovery preimage, restore/reconciliation receipts and sanitized rollback proof for the current LWC candidate | Deferred; Astra blocks deployment until this exists |
