...
| OA-008 | LWC candidate deploy/restore support | Before promoting the lightweight demo path into reusable campaign infrastructure | Productize the operation archive/manifest into the normal metadata recovery host, including durable authority, fenced dispatch and replay-safe recovery for LWC bundles | Exact source package, check-only result, durable recovery preimage, restore/reconciliation receipts and sanitized rollback proof for each current LWC candidate | Partially satisfied for demo operation `restore-20260911T155750Z-1e31703f`; reusable production-grade campaign support remains deferred |
